/* EPei - auth.js v3.0 JWT */
'use strict';
let currentUser = null;

async function initAuth() {
  const res = await API.getMe();
  if (res.success && res.user) { currentUser = res.user; updateNavForUser(); }
}
...
async function handleLogin(e) {
  ...
  showLoading(false);
  if (res.success) {
    currentUser = res.user; updateNavForUser();
...
async function handleRegister(e) {
  ...
  } else { showToast(res.message || 'Đăng ký thất bại', 'error'); }
}

async function handleLogout() {
  await API.logout(); currentUser = null; updateNavForUser();
  showToast('Đã đăng xuất', 'info'); showPage('home');
}
...
function updateNavForUser() {
  ...
  if (!btn) return;
  if (currentUser) {
    btn.innerHTML = `<i class="fas fa-user-circle"></i> ${currentUser.name.split(' ').pop()}`;
